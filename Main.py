...
def startNPR():
    ...
    if len(listOfPossiblePlates) == 0:  # if no plates were found
        ...
        global numberPlateInString
        numberPlateInString = licPlate.strChars
...
def drawRedRectangleAroundPlate(imgOriginalScene, licPlate):
    getValues = cv2.boxPoints(licPlate.rrLocationOfPlateInScene)  # get 4 vertices of rotated rect
    index00, index01 = getValues[0]
    index10, index11 = getValues[1]
    index20, index21 = getValues[2]
    index30, index31 = getValues[3]
    p2fRectPoints = [0, 0, 0, 0]
    p2fRectPoints[0] = (int(index00), int(index01))
    p2fRectPoints[1] = (int(index10), int(index11))
    p2fRectPoints[2] = (int(index20), int(index21))
    p2fRectPoints[3] = (int(index30), int(index31))
    cv2.line(imgOriginalScene, p2fRectPoints[0], p2fRectPoints[1], SCALAR_RED, 2)  # draw 4 red lines
